    ...
    <div class="hotProducts" :class="{container: styleObj.container}">
    	...
    	<div class="products">
			<nuxt-link :to="{name: 'goods-id', params: {id: item.id}}" v-for="item in hotData" :key="item.id" :style="styleObj">
                ...
                <div class="countDown">
                    <!-- <CountDown></CountDown> -->
                    <count-down v-on:start_callback="countDownS_cb(1)" v-on:end_callback="countDownE_cb(1)" :currentTime="item.currentTime" :startTime="item.currentTime" :endTime="item.endTime" :tipText="'距离开始文字1'" :tipTextEnd="'距离结束文字1'" :endText="'结束自定义文字2'" :dayTxt="'Days '" :hourTxt="':'" :minutesTxt="':'" :secondsTxt="''"></count-down>
